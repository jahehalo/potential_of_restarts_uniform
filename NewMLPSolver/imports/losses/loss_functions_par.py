...
def ks_par(y_true, y_pred):
    ...
    #foo = tf.Print(1.0+1.0,[K.log(scale),K.log(scalePred)])
    
    # if runtime > -scale/shape then create the factor a, that, multiplied with scale, fulfills the requirements
    #a = K.switch(K.less_equal(runtime,-scale/shape),K.ones_like(scale),-runtime*shape/scale)
    
    #scale = K.switch(K.less(shape,K.zeros_like(shape)),a*scale,scale)
    
    #diff = K.switch(K.less(shape,K.zeros_like(shape)),K.zeros_like(shape),a*scale-scale)/(10**8)
    ...
    with tf.control_dependencies([assert_scale, assert_shape]):
        ...
        res =  ks(runtime,shapePred, scale, empiric) + ks(runtime,shape, scalePred, empiric)# + (foo-2)  + ks(runtime,shape, scale, empiric)
        return K.sum(K.abs(res), axis = -1)+1*K.sum(K.abs(shape-shapePred), axis = -1)
        
        
        #bracket = 1 + (shape * runtime / scale)
        #res = 1-K.pow(bracket,-1/shape)

        #bool1 = K.greater(runtime,-scale/shape)
        #bool2 = K.less(shape,K.zeros_like(shape))
        #res = K.switch(tf.logical_and(bool1,bool2),empiric-K.ones_like(empiric),empiric - res)
        # shape_res = K.clip(K.abs(shape-shapePred), 0, 1)
        # scale_res = K.clip(K.abs(scale-scalePred)/(10**8), 0, 1)
        
        #result = K.abs(res) + 1.2*K.abs(shape_res)# + K.abs(diff) + K.abs(scale_res)
