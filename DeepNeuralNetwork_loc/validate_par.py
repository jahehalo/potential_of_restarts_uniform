...
import functions
import losses.loss_functions_par as loss
import losses.loss_functions_logn as log_loss
import metrics.metrics_par as metrics
...
feat_label = functions.createSampleData(distribution='pareto')
...
parX, parY = functions.sample(feat_label, k=1/k, seed=identification+1)
valX,validY = functions.createRuntimeData(distribution='pareto')
distValid = validY
...
c = 9
#for c in range(k):
## Train network for pareto

parXtest = list(it.chain(it.chain(* parX[c])))
parYtest = list(it.chain(it.chain(* parY[c])))
parXtrain = list(it.chain(* it.chain(*(parX[:c]+parX[c+1:]))))
parYtrain = list(it.chain(* it.chain(*(parY[:c]+parY[c+1:]))))


#print(weibX)
parXtrain, mean, variance = functions.normalize(parXtrain)#, mean, variance)
parXtest, mean, variance = functions.normalize(parXtest, mean, variance)
validX, mean, variance = functions.normalize(valX,mean,variance)

distTest = parYtest
parYtest = [ [x[0],x[1],x[2], x[3]] for x in parYtest ]
parYtrain = [ [x[0],x[1],x[2], x[3]] for x in parYtrain ]

filepath = "./chosen_models/pareto_l_1.0_d_0.3_g_0.05_gh_0.2_loss_1.0_neurons_32_decay_0.9_id_11013_run_9.h5"
if os.path.isfile(filepath):
    model2 = load_model(filepath,custom_objects={"exp":K.exp,"ks_par":loss.ks_par,"rmse_shape_par":metrics.rmse_shape_par,"log_rmse_scale_par":metrics.log_rmse_scale_par})

print("mean: ", mean, "variance: ", variance)

ev= model2.evaluate(x=validX, y=validY, batch_size=batchSize, verbose=0)
print("validX: ", ev)
ev= model2.evaluate(x=parXtest, y=parYtest, batch_size=batchSize, verbose=0)
print("testData:", ev)
...
for i in range(0,lines):
    preds.append(model2.predict(numpy.asarray([parXtest[300*i]]),verbose=0))
...
s = "model_testData.h5"
# write prediction
with open("./results/predictions/par"+ s, 'w') as f:
    for p in preds:
...
with open("./results/estimations/par"+s, 'w') as f:
    for like in maxLs:
        f.write(str(like).replace("[","").replace("]","").replace(",","") + "\n")

# write runtimes
with open("./results/runtimes/par"+s, 'w') as f:
    for r in runs:
...
# create list of runtimes to print
runsRaw = [x[0] for x in distValid]
