            ...
            for feat in files:
                ...
                if os.path.isfile(distPath + dist):
                    ...
                    with open(distPath + dist, 'r') as f:
                        ...
                        a= []
                        for run in runtimes:
                            # Readjust the runtimes by subtracting the location parameter.
                        ...
                        for run in a:
                            ...
                            weibtrainingLabels.append([run,i/l,shape,scale])
                            i = i+1               
...
def createSampleData():
    """
        This function returns the data for all Weibull distributions (according to the higher p-value in the KS test).
    """
    run_list = []
    feats = []
    label = []
                ...
                if os.path.isfile(distPath + dist):
                    ...
                    with open(distPath + dist, 'r') as f:
                        ...
                        if float(lines[9].split()[1]) < float(lines[17].split()[1]):
                            # Only Weibull distributions (according to the higher p-value in the KS test) should be considered.
                            # If this part is exectued, then the lognormal distribution is the better fitting distribution.
                            continue 
                        loc = int(lines[1].split()[1])
                        scale = float(lines[2].split()[1])
                        shape = float(lines[5].split()[1])
                        ...
                        for run in a:
                            run_list.append(a)
                            X.append(features)
                            Y.append([run-loc,i/l,shape,scale, loc])
                            i = i+1
